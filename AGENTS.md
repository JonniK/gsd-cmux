...
8c. **Team names are derived, not passed.** OMC 4.x has **no `--team-name` flag** on `omc team` — `parseTeamArgs` (cli.cjs ~81858) accepts only `--json` and `--new-window`. The team name is always `slugifyTask(task)` (cli.cjs ~81798): `task.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"").slice(0,30)`. To get a predictable name, pick a string `T` that is slugify-invariant — matches `[a-z0-9][a-z0-9-]{0,28}[a-z0-9]`, no `--` runs, ≤30 chars — and pass `T` itself as the task-description argument. Then OMC uses `T` verbatim. Do **not** pass a prose bootstrap as the task argument: OMC slugifies its first 30 chars into the team name, producing an unpredictable / invalid slug (past symptom: `"Invalid team name: 2laude-executor-you-are-a-gsd-"`). The per-worker lifecycle comes from the `gsd-omc-bridge` skill auto-loaded via `agent_skills` on the `gsd-executor` agent-type (wired by `setup-gsd-omc.sh`). After spawn, assert `omc team status "$T"` succeeds before any `omc team api` call against `$T`; slugify divergence otherwise silently misroutes every subsequent API call.
